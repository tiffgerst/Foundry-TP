...
//             depositUserA[i] = (tokenAmountA[i] * getPriceInUSD(feedAddress[i])) / (10 ** feedContract[1].decimals());
//             sumDepositA += depositUserA[i];
//             depositUserB[i] = (tokenAmountB[i] * getPriceInUSD(feedAddress[i])) / (10 ** feedContract[1].decimals());
//             sumDepositB += depositUserB[i];
//             expectPoolAum[i] = depositUserA[i] + depositUserB[i];
//             totalAum += expectPoolAum[i];
//             assertEq(depositUserA[i] + depositUserB[i], ITokenPool(pool).getPoolValue());
//             assertEq(expectPoolAum[i], ITokenPool(pool).getPoolValue());
//         }
